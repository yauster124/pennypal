...
import { useGetMonthlyTotals } from "../../api/get-monthly-totals";
...
export const MonthlyTotalsSection = () => {
    ...
    const getMonthlyTotals = useGetMonthlyTotals(
    ...
    return (
        <Card className="@container/card flex-1">
            <CardHeader>
                ...
                <CardDescription>
                    <span className="hidden @[540px]/card:block">
                        {timeRange === "6m" && "Totals per month over the last 6 months"}
                        {timeRange === "1y" && "Totals per month over the last year"}
                        {timeRange === "5y" && "Totals per month over the last 5 years"}
                        {timeRange === "max" && "Totals per month since inception"}
                    </span>
            ...
            <CardContent>
                {getMonthlyTotals.data && getMonthlyTotals.data.length > 0 ? (
                    <MonthlyTotalsChart
                        chartData={getMonthlyTotals.data}
                        type="expense"
                    />
                ) : getMonthlyTotals.data && getMonthlyTotals.data.length <= 0 ? (
